			  ...
			  unsigned int relation)
{
	...
	/* Check if there need to change PLL */ 
		if((apll_old != apll_values[index]) || (index <= L0) || 
		(freqs.old >= s5pv210_freq_table[L0].frequency))
		pll_changing = 1;
...
{
    return s5pv210_freq_table[L1].frequency; // 800 mhz
}
...
static ssize_t user_max_write(struct device * dev, struct device_attribute * attr, const char * buf, size_t size)
{
    ...
    if(sscanf(buf, "%u\n", &data) == 1)
  {
      if (data >= s5pv210_freq_table[L4].frequency && data <= s5pv210_freq_table[L0].frequency)
    {
        i = 0;
          while (s5pv210_freq_table[i].frequency != CPUFREQ_TABLE_END) {
      index = s5pv210_freq_table[i].index;
...
// only save the actual voltages
  L1_int_volt = dvs_conf[4].int_volt;
  L2_int_volt = dvs_conf[5].int_volt;
  L3_int_volt = dvs_conf[6].int_volt;
  ...
  L2_int_volt = dvs_conf[5].int_volt;
  L3_int_volt = dvs_conf[6].int_volt;
  dvs_conf[4].int_volt = L1_int_volt - 50000;
  dvs_conf[5].int_volt = L2_int_volt - 50000;
  dvs_conf[6].int_volt = L3_int_volt - 50000;
...
int i;
  ...
  clkdiv_val[5][3] = 1;
  clkdiv_val[6][3] = 1;
  dvs_conf[4].int_volt = L1_int_volt;
  dvs_conf[5].int_volt = L2_int_volt;
  dvs_conf[6].int_volt = L3_int_volt;
