...
extern unsigned long cpuOC0freq(void);
extern unsigned long cpuOC1freq(void);
extern unsigned long cpuOC2freq(void);
extern unsigned long cpuOC3freq(void);
extern unsigned long cpuL0freq(void);
...
static ssize_t store_sleep_ideal_freq(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	...
	if (res >= 0 && input >= 0) {
#ifdef CONFIG_LIVE_OC
	if(input > 0 && input <= cpuL4freq())
	    sleep_ideal_freq = cpuL4freq();
	else if(input > cpuL4freq() && input <= cpuL3freq())
	    sleep_ideal_freq = cpuL3freq();
	else if(input > cpuL3freq() && input <= cpuL2freq())
	    sleep_ideal_freq = cpuL2freq();
	else if(input > cpuL2freq() && input <= cpuL1freq())
	    sleep_ideal_freq = cpuL1freq();
	else    sleep_ideal_freq = cpuL3freq();
#else
...
#ifdef CONFIG_LIVE_OC
	if(input > cpuOC2freq())
	    sleep_wakeup_freq = cpuOC1freq();
	else if(input > cpuOC3freq() && input <= cpuOC2freq())
	    sleep_wakeup_freq = cpuOC2freq();
	else if(input > cpuL0freq() && input <= cpuOC3freq())
	    sleep_wakeup_freq = cpuOC3freq();
	else if(input > cpuL1freq() && input <= cpuL0freq())
	    sleep_wakeup_freq = cpuL0freq();
	else if(input > cpuL2freq() && input <= cpuL1freq())
...
#ifdef CONFIG_LIVE_OC
	if(input > cpuOC2freq())
	    awake_ideal_freq = cpuOC1freq();
	else if(input > cpuOC3freq() && input <= cpuOC2freq())
	    awake_ideal_freq = cpuOC2freq();
	else if(input > cpuL0freq() && input <= cpuOC3freq())
	    awake_ideal_freq = cpuOC3freq();
	else if(input > cpuL1freq() && input <= cpuL0freq())
	    awake_ideal_freq = cpuL0freq();
	else if(input > cpuL2freq() && input <= cpuL1freq())
